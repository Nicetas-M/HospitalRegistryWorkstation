...
namespace АРМ_реєстратури_лікарні
{
	public partial class Contingents : Form
	{
		...
		{
			SqlDataAdapter sqlDataAdapter = new SqlDataAdapter("SELECT * FROM Contingents", sqlConnection);
		...
		private void Contingents_Load(object sender, EventArgs e)
		{
			LoadTable();
		...
		private void dataGridView1_Click(object sender, EventArgs e)
		{
			...
			if (dgv.SelectedRows != null)
			{
				...
				Contingent_TextBox.Text = Convert.ToString(dgv.SelectedRows[0].Cells["Contingent"].Value);
				oldIDContingent = Contingent_ID_TextBox.Text;
			}
		}
		...
		{
			SqlCommand command = new SqlCommand("Add_Contingent", sqlConnection);
			...
			{
				ParameterName = "@contingent",
				Value = Contingent_TextBox.Text
			};
		...
		{
			SqlCommand command = new SqlCommand("Update_Contingent", sqlConnection);
			command.CommandType = CommandType.StoredProcedure;

			SqlParameter oldIDParam = new SqlParameter()
			{
				ParameterName = "@old_id_contingent",
				Value = oldIDContingent
			};
			command.Parameters.Add(oldIDParam);

			SqlParameter newIDParam = new SqlParameter()
			{
				ParameterName = "@new_id_contingent",
				Value = Contingent_ID_TextBox.Text
			};
		...
		{
			SqlCommand command = new SqlCommand("Delete_Contingent", sqlConnection);
			command.CommandType = CommandType.StoredProcedure;
		...
		{
			new Contingents_Report() { sqlConnection = sqlConnection }.Show();
		}
	}
}

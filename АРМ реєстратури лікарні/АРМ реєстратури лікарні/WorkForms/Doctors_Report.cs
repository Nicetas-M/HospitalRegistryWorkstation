...
namespace АРМ_реєстратури_лікарні
{
	public partial class Doctors_Report : Form
	{
		...
		{
			SqlDataAdapter adapter = new SqlDataAdapter("SELECT * FROM Doctors", sqlConnection);
			DataSet dataSet = new DataSet();
			adapter.Fill(dataSet);
			adapter.Dispose();
			doctorsDataSetBindingSource.DataSource = dataSet.Tables[0];
